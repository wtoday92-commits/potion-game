...
# Уровень N достигнут при value >= REP_LEVELS[N-1]. Пороги снижены: со старыми
# и приростом 7 за годный заказ до пятого уровня было 38 заказов С ОДНИМ гостем,
# то есть около 26 циклов на одного из 27 — пассивки ур.4-5 не существовали.
const REP_LEVELS := [22, 55, 100, 155, 220]
